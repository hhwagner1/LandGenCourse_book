...
#### a) How to install (or update) `LandGenCourse`
```{r}
...
- Use the add-in `Install Packages` to bulk install most or all packages needed to run the worked examples.
- Simply click on the add-in `Install Packages` (alternatively, you could run this code: `installDGS()`)
...
- If the installation stops with an error `API rate limit exceeded`, the best thing to do is to wait 30 min and run the add-in `Install Packages` again. Don't worry, this won't start the installation process again, it will resume where it broke off. 
...
- If you do so, the second line above will open the file `.Renviron` in RStudio. Enter a `GITHUB_PAT="myPAT"` on the last line (replace myPAT by the PAT you generated using the first line). Make sure to include ian empty line after it! Save the file and restart R. 



#### b) How to use the R package `LandGenCourse`

The package installs four more Add-ins in RStudio. Each will provide you with some dropdown menu choices.
